...
rows = 3
columns = 19
bubble_count = rows*columns # add phase 5
moving = False
game_over = False
shot_count = 0
color = ""
...
# check if neighboring bubble is in play and the same color - add phase 5
...
# recursively deletes clusters of the same colored balls - add phase 5
...
# ball collision - update phase 5
def ball_collision(sprite, hit_sprite):
    global moving, game_over
    if hit_sprite != arrow:
        moving = False 
        if hit_sprite.get_color() == color:
            del_group(hit_sprite, bubbles.index(hit_sprite)) # add phase 5
            if bubble_count == 0: # add phase 5
